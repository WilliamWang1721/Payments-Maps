...
import { inferMerchantNameWithOpenRouter } from "@/lib/openrouter-merchant";
...
export function AddLocationMapPicker({
  ...
  const applyPoint = async (nextLng: number, nextLat: number, options?: ApplyPointOptions) => {
    ...
    const sourceLabel = options?.sourceLabel || "Map selection";
    if (!options?.preserveDebugFlow) {
      startDebugFlow(requestId, sourceLabel, `${nextLat.toFixed(6)}, ${nextLng.toFixed(6)}`);
    }
    appendDebugEvent(requestId, "success", "Coordinates updated", `${nextLat.toFixed(6)}, ${nextLng.toFixed(6)}`);
    ...
    if (options?.resolveAddress === false) {
      appendDebugEvent(requestId, "info", "Address resolution skipped");
      lastSyncedPointRef.current = { lat: nextLat, lng: nextLng };
      return;
    }

    appendDebugEvent(requestId, "running", "Resolving address", "Calling AMap reverse geocoder...");
    ...
    if (nextAddress.address) {
      appendDebugEvent(requestId, "success", "Address resolved", nextAddress.address);
    } else {
      appendDebugEvent(requestId, "error", "Address resolution returned empty", "AMap did not return a formatted address.");
    }

    if (!nextAddress.address) {
      appendDebugEvent(requestId, "info", "Merchant inference skipped", "No formatted address was available.");
      onMerchantInferenceStateChange?.({ status: "idle" });
      return;
    }

    if (!autoReadMerchantNameEnabled) {
      appendDebugEvent(requestId, "info", "Merchant inference disabled", "This Beta feature is turned off in Settings > Add Location.");
      onMerchantInferenceStateChange?.({ status: "disabled" });
      return;
    }

    appendDebugEvent(requestId, "running", "Running AI merchant inference", "Calling Supabase Edge Function infer-merchant-name...");
    void inferMerchantNameWithOpenRouter({
      ...
      if (error) {
        appendDebugEvent(requestId, "error", "AI merchant inference failed", error);
        onMerchantInferenceStateChange?.({ status: "error", error });
      } else if (merchantName) {
        appendDebugEvent(requestId, "success", "Merchant name detected", merchantName);
        onMerchantInferenceStateChange?.({ status: "success", merchantName });
      } else {
        appendDebugEvent(requestId, "success", "AI marked this point as a non-merchant location");
      ...
      try {
        ...
        map.on("click", (event: any) => {
          ...
          if (typeof nextLng === "number" && typeof nextLat === "number") {
            void applyPoint(nextLng, nextLat, { center: false, sourceLabel: "Map click selection" });
          }
        });
        ...
        marker.on("dragend", (event: any) => {
          ...
          const nextLat = event?.lnglat?.getLat?.();
          if (typeof nextLng === "number" && typeof nextLat === "number") {
            void applyPoint(nextLng, nextLat, { center: false, sourceLabel: "Marker drag update" });
          }
        });
        ...
        setMapError(error instanceof Error ? error.message : "AMap failed to load.");
        setMapLoading(false);
      }
    };
  ...
  const handleLocate = async (): Promise<void> => {
    if (!navigator.geolocation) {
      ...
      geocodeRequestIdRef.current = requestId;
      startDebugFlow(requestId, "Browser geolocation");
      appendDebugEvent(requestId, "error", "Browser geolocation failed", "Geolocation is not supported in this browser.");
    ...
    const requestId = geocodeRequestIdRef.current + 1;
    geocodeRequestIdRef.current = requestId;
    startDebugFlow(requestId, "Browser geolocation", "Requesting current coordinates from the browser.");
    appendDebugEvent(requestId, "running", "Waiting for browser location permission and coordinates");
    ...
    try {
      ...
      appendDebugEvent(
        ...
        error instanceof Error ? error.message : "Unable to get your location. Please check browser permissions."
      );
      setLocateError(error instanceof Error ? error.message : "Unable to get your location. Please check browser permissions.");
    } finally {
      setLocating(false);
    }
  };
    ...
    <div className="relative h-full w-full">
      <div className="absolute bottom-6 right-6 z-10 flex flex-col items-end gap-2">
        ...
        >
          ...
          <span>{locating ? t("Locating...") : t("Locate")}</span>
        </button>
      </div>
      ...
      {mapLoading ? (
        <div className="absolute inset-0 z-[1] flex items-center justify-center bg-[linear-gradient(180deg,#fafafacc,#f3f4f6f0)]">
          <div className="flex flex-col items-center gap-3 text-[var(--muted-foreground)]">
            ...
            <p className="text-sm">{t("Loading AMap layer...")}</p>
          </div>
        </div>
      ) : null}
      ...
      {mapError ? (
        <div className="absolute inset-0 z-[2] flex items-center justify-center bg-[linear-gradient(180deg,#fafafae6,#f3f4f6f2)] px-6">
          <div className="max-w-[420px] rounded-m border border-[#f3bbb2] bg-white px-5 py-4 text-center shadow-[0_20px_60px_-32px_rgba(15,23,42,0.28)]">
            <p className="text-sm font-semibold text-[#8f291a]">{t("AMap layer failed to load")}</p>
