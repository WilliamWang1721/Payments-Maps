...
function createMcpServer(sessionToken: string): McpServer {
  const server = new McpServer(
    ...
    {
      instructions:
        "Fluxa Map business MCP server. Use structured tools to read and update Fluxa business data. " +
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      return withAudit(liveSession, "create_personal_card", args, async () => {
        const card = await fluxaService.createPersonalCard(liveSession.userId, args);
        return {
